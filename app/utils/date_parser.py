...
def test_nba_date_parser():
    """Test the NBA date parser with various expressions."""
    parser = NBADateParser()
    ...
    test_queries = [
        ...
        "Anthony Davis since January",
        "Jimmy Butler after last month",
        
        # Explicit dates
        "Paolo Banchero since 2024-01-15",
        "Shai since January 15, 2024",
        "Nikola Jokic from 01/15/2024",
        
        # Complex expressions
        "Victor Wembanyama since the beginning of February",
        "Jayson Tatum after New Year",
        "Kawhi Leonard since last week",
    ]
    
    print("NBA Date Parser Test Results:")
    print("=" * 50)
    
    for query in test_queries:
        result = parser.get_date_components(query)
        print(f"Query: '{query}'")
        print(f"  Date Filter: {result['date_filter']}")
        print(f"  Original Expression: '{result['original_expression']}'")
        print(f"  Date Type: {result['date_type']}")
        print(f"  Confidence: {result['confidence']}")
        print()


if __name__ == "__main__":
    test_nba_date_parser() 
...
from app.services.player_pool import PlayerPoolReader, PoolPlayer
...
class MatchupSelectionService:
    ...
    def get_selection(self, *, game_id: str, player_id: int) -> dict[str, Any]:
        ...
        pool = self.player_pool.get_pool(season=season, game_ids=(game_id,))
        player = next(
            (
                candidate
    ...
    def _market_values(
        ...
        return {
            market: sum(
                ...
                for component in self._statistics[market].components
            )
            for market in markets
        }

...
| `CatalogSettings` | `athlete_freshness_days` and `player_game_log_min_active_players_per_team_game` (positive whole-number `int` values), plus `event_max_age_hours`, `event_match_window_hours`, `slate_schedule_max_age_hours`, and `player_game_log_max_age_hours` (exact `Decimal` hours) | `ATHLETE_CATALOG_FRESHNESS_DAYS` (default `7`), `PLAYER_GAME_LOG_MIN_ACTIVE_PLAYERS_PER_TEAM_GAME` (default `5`), `EVENT_CATALOG_MAX_AGE_HOURS` (default `72`), `EVENT_MAPPING_MATCH_WINDOW_HOURS` (default `6`), `SLATE_SCHEDULE_MAX_AGE_HOURS` (default `30`), `PLAYER_GAME_LOG_MAX_AGE_HOURS` (default `30`) |

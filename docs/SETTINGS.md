...
| `CatalogSettings` | `athlete_freshness_days` (whole days, `int`), `event_max_age_hours`, `event_match_window_hours`, `slate_schedule_max_age_hours`, and `player_game_log_max_age_hours` (exact `Decimal` hours) | `ATHLETE_CATALOG_FRESHNESS_DAYS` (default `7`), `EVENT_CATALOG_MAX_AGE_HOURS` (default `72`), `EVENT_MAPPING_MATCH_WINDOW_HOURS` (default `6`), `SLATE_SCHEDULE_MAX_AGE_HOURS` (default `30`), `PLAYER_GAME_LOG_MAX_AGE_HOURS` (default `30`) |

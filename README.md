...
| `PROJECTION_ARCHIVE_READ_PROVIDER` | No | `dabble`; recorder compatibility scope when `DFS_ENABLED_PROVIDERS` is empty; it does not enable reads or failure fallback |
...
never fans out by player and has no request-time provider fallback.
...
- Protected routes include `GET /api/games/slate`, `GET /api/games/matchup`, `GET /api/games/game_logs`, `POST /api/nl-query`, and most `/api/user/*` routes.

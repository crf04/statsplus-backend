...
def test_projection_archive_gate_refuses_an_unmigrated_application_database(
    ...
    with pytest.raises(
        ConfigurationError,
        match="require migrations 040_projection_archive, 041_projection_archive_transitions, 044_projection_closing_sets, and 045_projection_mapping_replay.*missing",
    ):
        build_dependencies(settings)

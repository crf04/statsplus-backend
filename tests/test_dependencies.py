...
def test_projection_archive_gate_refuses_an_unmigrated_application_database(
    ...
    with pytest.raises(
        ConfigurationError,
        match="require migrations 040_projection_archive, 041_projection_archive_transitions, 044_projection_closing_sets, 045_projection_mapping_replay, and 046_projection_observation_prices.*missing",
    ):
        build_dependencies(settings)
...
def test_a_no_eviction_redis_with_no_maxmemory_warns_the_expected_policy(
    ...
    assert redis_client.config_calls == [("maxmemory-policy", "maxmemory")]

...
def test_one_targets_database_error_leaves_the_other_backtests_ok_on_postgres(
    pg_engine, user_service
):
    """Postgres aborts the whole transaction on a statement error; each
    Target's savepoint confines that to the Target that raised it."""
    ...
    from tests.test_target_backtest import (
        BOS,
        CORNER_THREE,
        LEBRON,
        LOW_RIM,
    ...
    for opponent, qualifiers in (
        ...
        user_service.create_target(
            uid, opponent=opponent, qualifiers=qualifiers, note=None
        )
    listed = [target["id"] for target in user_service.list_targets(uid)]
    seams = _two_games()
    ...
    payload, _ = service.backtest_all(uid)

    items = payload["backtests"]
    assert [item["target_id"] for item in items] == listed
    # BOS is newest, so it runs first and its error would otherwise abort
    # the transaction every later Target reads on.
    assert items[0]["status"] == "error"
    assert items[0]["error"]["code"] == "operation_failed"
    assert [item["status"] for item in items[1:]] == ["ok", "ok"]
    for item in items[1:]:
        assert [p["canonical_id"] for p in item["backtest"]["players"]] == [LEBRON]

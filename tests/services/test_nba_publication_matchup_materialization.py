...
def _rows(metric_key: str) -> tuple[PublicationTeamWindowRow, ...]:
    return tuple(
        PublicationTeamWindowRow(
            team_id=team_id,
            ...
            game_count=1,
            per48={metric_key: float(team_id)},
            league_average={metric_key: 15.5},
            population_sigma={metric_key: 8.655},
            competition_rank={metric_key: team_id},
        )
        for team_id in range(1, 31)
    )


def _reader(*, unavailable: frozenset[str] = frozenset()):
    metrics = {
    ...
    for stream_key, metric_key in metrics.items():
        ...
        reads[stream_key] = PublicationRead(
            ...
            season="2025-26",
            cutoff="2025-10-15T00:00:00+00:00",
            version=2,
            status="active",
            freshness="stale",
            age_seconds=90000,
...
def test_nba_publications_persist_independent_facts_with_lineage(tmp_path):
    ...
    assert fact.publication_id == "publication-exact_shot_zones_opponent_season"
    assert fact.publication_cutoff == "2025-10-15T00:00:00+00:00"
    assert fact.publication_freshness == "stale"
    ...
    assert observations["shot_zones"].publication_id == fact.publication_id
